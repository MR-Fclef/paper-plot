...
figure_poi('深圳市罗湖区公交地铁.xls', 0, 3, 4, 'bo')
figure_poi('深圳市罗湖区购物.xls', 0, 6, 7, 'go')
figure_poi('深圳市罗湖区酒店.xls', 0, 6, 7, 'co')
figure_poi('深圳市罗湖区旅游景点.xls', 0, 6, 7, 'mo')
figure_poi('深圳市罗湖区医疗.xls', 0 ,6, 7, 'ro')
figure_poi('深圳市罗湖区餐厅.xls', 0, 6, 7, 'yo')
figure_poi('深圳市罗湖区停车场.xls', 0, 6, 7,'ko')
plt.legend(['public traffic','shopping mall','hotel','scenic spot','hospital','restaurant','parking lot'], fontsize=23)
plt.xlabel('Longitude', fontsize=25)
plt.ylabel('Latitude', fontsize=25)
plt.title('POI of Shenzhen City Luohu District', fontsize=25)
plt.xticks(fontsize=23)
plt.yticks(fontsize=23)
fig = plt.gcf()
fig.set_size_inches(25, 15)
fig.savefig('POI of Shenzhen City Luohu District.png', dpi=300)
plt.show()
...
add_spot('深圳市罗湖区公交地铁.xls', 0, 3, 4, 1)
add_spot('深圳市罗湖区购物.xls', 0, 6, 7, 2)
add_spot('深圳市罗湖区酒店.xls', 0, 6, 7, 3)
add_spot('深圳市罗湖区旅游景点.xls', 0, 6, 7, 4)
add_spot('深圳市罗湖区医疗.xls', 0 ,6, 7, 5)
add_spot('深圳市罗湖区餐厅.xls', 0, 6, 7, 6)
add_spot('深圳市罗湖区停车场.xls', 0, 6, 7, 7)
...
单独统计停车场的经纬度信息
'''
df = pd.read_excel('深圳市罗湖区停车场.xls')
lat = df.ix[0:, 6]
lng = df.ix[0:, 7]
x = []
y = []
...
title = ['public transpot','shopping','hotel','view spot','hospital & clinic','restaurant','parking lots','type']
...
for i in range(len(multi_type)):
    for j in range(7):
        sheet.write(i+1, j, multi_type[i][j])
    sheet.write(i+1, 7, 'type'+str(parking_type[i]))
wbk.save('停车场七维向量.xls')

'''
从刚才保存的“停车场六维向量.xls”表中提取数据绘制平行坐标系
'''
data = pd.read_excel('停车场七维向量.xls')
plt.subplot(111,axisbg='ghostwhite')
parallel_coordinates(data,'type',color=['#aaaaaa', '#d87a80', '#ffb980','#5ab1ef','#b6a2de','#2ec7c9'],linewidth=3)
...
fig.savefig('POI Clustering of 7d.png', dpi=200, bbox_inches='tight')
plt.show()

'''
给停车场加type属性，并统计各个类型停车场的数目
'''
rd = xlrd.open_workbook('深圳市罗湖区停车场.xls', formatting_info=True)
wb = copy(rd)
sheet = wb.get_sheet(0)
sheet.write(0,8,'type')
for i in range(len(parking_type)):
    sheet.write(i+1, 8, 'type'+str(parking_type[i]))
os.remove('深圳市罗湖区停车场.xls')
wb.save('深圳市罗湖区停车场.xls')
print('.........................................................................')
df = pd.read_excel('深圳市罗湖区停车场.xls')
type0 = []
type1 = []
